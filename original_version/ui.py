...
PIXEL_CSS = {
    ...
    /* Fix collaboration section and other cards */
...
def initialize_session_state():
    ...
    # Processing components with IBM Granite models
                ...
                try:
                    ...
                    if Config.GOOGLE_API_KEY:
                        ...
                        if quick_notes:
                            ...
                            st.session_state.quick_notes = "QuickNotes generation failed."
                    else:
...
with sidebar:
    ...
    st.markdown("<div class='section-title'>COLLABORATION</div>", unsafe_allow_html=True)
    st.markdown("""
    <div class='collaboration-card'>
        <div style='font-weight:600; color: var(--text); margin-bottom: 8px;'>Group Study</div>
        <p style='margin:6px 0; color: var(--text-muted);'>Invite friends and study together</p>
        <button class='collaboration-btn'>+ Join a friend</button>
    </div>
    """, unsafe_allow_html=True)
...
with main:
    ...
    if st.session_state.page == "Normal":
        ...
        if not st.session_state.pdf_uploaded:
            ...
            st.info("👈 Use the sidebar to upload your PDF file")
        else:
            ...
            if getattr(st.session_state, 'progressive_mode', False):
                ...
                if st.session_state.processed_data and 'total_text' in st.session_state.processed_data:
                    ...
                    # Generate general summary if not already done
                    if not hasattr(st.session_state, 'general_summary'):
                        with st.spinner("Generating document overview..."):
                            try:
                                # Use Gemini API for general summary
                                if Config.GOOGLE_API_KEY:
                                    ...
                                    if summary:
                                        ...
                                        st.session_state.general_summary = "Unable to generate general summary at this time."
                                else:
                                    st.session_state.general_summary = "API key required for general summary generation."
                            except Exception as e:
                                st.session_state.general_summary = "Unable to generate general summary at this time."
                                logger.error(f"Error generating general summary: {e}")

                    # Display general summary
                    if hasattr(st.session_state, 'general_summary') and st.session_state.general_summary:
                        st.text_area("", value=st.session_state.general_summary, height=200, disabled=True, label_visibility="collapsed")
                    else:
                        st.text_area("", value="Generating document summary... Please wait or refresh the page.", height=200, disabled=True, label_visibility="collapsed")
        ...
        if not st.session_state.pdf_uploaded:
            ...
            if st.session_state.quick_notes:
                ...
                if st.button("💾 Save Notes"):
                    st.success("Notes saved! (In a full implementation, these would be persisted)")
            else:
                st.info("Generating QuickNotes... Please wait.")
