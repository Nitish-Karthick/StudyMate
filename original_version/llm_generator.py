...
class LLMGenerator:
    ...
    def __init__(self, model_name: str = "microsoft/DialoGPT-small"):
        ...
        # Try to initialize local model, but don't fail if it doesn't work
        try:
            self._initialize_local_model()
        except Exception as e:
            logger.warning(f"Could not initialize LLM model: {str(e)}")
            logger.info("Will use simple text processing instead")
    ...
    def generate_quick_notes(self, full_text: str) -> str:
        """
        ...
        try:
            ...
            notes = []
            for i, point in enumerate(key_points[:8]):  # Limit to 8 points
                notes.append(f"• {point}")

            result = '\n'.join(notes)

            logger.info("Generated extractive QuickNotes")
            return result if result else "• Key concepts and information extracted from the document."
            ...
            try:
                ...
                return "• Document contains important study material."
    ...
    def generate_quiz_questions(self, text: str, num_questions: int = 10, use_api: bool = False) -> List[Dict]:
        ...
        try:
            ...
            # Use Gemini as primary since OpenRouter is having JSON parsing issues
            if use_api and Config.GOOGLE_API_KEY:
                logger.info("Using Gemini API for quiz generation")
...
IMPORTANT: Respond with ONLY a valid JSON array. No other text, no explanations, no markdown formatting.
...
Generate {num_questions} questions based on the actual content. Return only the JSON array:
"""
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    if isinstance(questions, list) and len(questions) > 0:
                        ...
                        logger.warning("Gemini returned empty or invalid question list")
...
"""
    ...
    def _generate_general_summary_with_gemini(self, text: str) -> str:
        """Generate a general summary of the entire document using Gemini API"""
        try:
            ...
            prompt = f"""
Please provide a comprehensive summary of this document in 3-4 paragraphs. Focus on:
...
"""
    ...
    def _generate_quick_notes_with_gemini(self, text: str) -> str:
        """Generate QuickNotes using Gemini API"""
        try:
            if not text or len(text.strip()) < 100:
            ...
            prompt = f"""
Create concise study notes from this document. Format as bullet points covering:
- Key concepts and definitions
- Important facts and figures
- Main topics and themes
- Critical information to remember

Keep it brief and focused on the most important points for studying.
...
"""
            ...
            if response.status_code == 200:
                ...
                logger.info("Generated QuickNotes using Gemini API")
                return notes
            else:
